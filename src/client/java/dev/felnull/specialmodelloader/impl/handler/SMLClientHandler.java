...
public class SMLClientHandler {
    ...
    private static CompletableFuture<Predicate<Identifier>> provideSimpleLoadScopePredicate(
            ResourceManager resourceManager, Executor executor) {
        BiPredicate<ResourceManager, Identifier> predicate = SpecialModelLoaderEvents.LOAD_SCOPE.invoker()
                .provideLoadScopePredicate();
        return CompletableFuture.supplyAsync(() -> loc -> predicate.test(resourceManager, loc), executor);
    }

    private static CompletableFuture<Predicate<Identifier>> provideManualLoadScopePredicate(
            ResourceManager resourceManager, Executor executor) {
        return CompletableFuture.supplyAsync(() -> {

            ImmutableSet.Builder<Pattern> patternsBuilder = new ImmutableSet.Builder<>();

            resourceManager.listResources(MANUAL_LOAD_SCOPE_RESOURCE_NAME, loc -> loc.getPath().endsWith(".json"))
                    .forEach((location, resource) -> {
                        try (Reader reader = resource.openAsReader()) {
                            JsonObject jo = GSON.fromJson(reader, JsonObject.class);
                            loadManualLoadScope(patternsBuilder, jo);
                        } catch (Exception e) {
                            SpecialModelLoader.LOGGER.error(
                                    "Error occurred while loading model load scope resource json {}", location, e);
                        }
                    });
    ...
    private static void loadManualLoadScope(ImmutableSet.Builder<Pattern> builder, JsonObject jsonObject) {
        ...
        if (!jsonObject.has("version") || !jsonObject.get("version").isJsonPrimitive()
                || !jsonObject.getAsJsonPrimitive("version").isNumber()) {
